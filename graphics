...
#
#
# Version 6.2 modifications:
#    - Automatic HiDPI font scaling wasn't working, so I fixed it.
#    - allowed setFill/setOutline to take None as a color (and translate it to empty string "")
#    - Added GraphicsObject getShapeSize() -> width,height tuple (need to add to PDF?)
#
...
class GraphWin(_tk.Canvas):
    ...
    def checkMouse(self):
        ...
        not been clicked since last call (or the window is closed)"""
        if self.isClosed():
            return None #seems more sensible
...
try:
    ...
    _HIDPI_FONT_SCALING_RATIO = 100.0 / _font.Font(family="Courier",size=100,weight="normal").measure('A')
    if _HIDPI_FONT_SCALING_RATIO < 0.25:
        _HIDPI_FONT_SCALING_RATIO = 0.25
    elif _HIDPI_FONT_SCALING_RATIO > 4.0:
        _HIDPI_FONT_SCALING_RATIO = 4.0
except:
...
class GraphicsObject:
    ...
    def getShapeSize(self):
        return (self.getShapeWidth(), self.getShapeHeight())
...
class Image(GraphicsObject):
    ...
    def getWidth(self):
        ...
        return self.getShapeWidth()

    def getHeight(self):
        """Returns the height of the image in pixels"""
        return self.getShapeHeight()
